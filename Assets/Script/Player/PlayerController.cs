...
public class PlayerController : MonoBehaviour
    {
        ...
        private void CrouchUpdate()
        {
            ...
            initCameraPosition.y = m_StandHeight;
            finalCameraPosition = m_Camera.transform.localPosition;
            finalCameraPosition.y = m_CrouchHeight;

            if (Input.GetButtonDown("Sneak")) { if (m_Camera.transform.localPosition.y == 0.8f) { crouchStart = Time.time; } }
            if (Input.GetButtonUp("Sneak")) { if (m_Camera.transform.localPosition.y == 0.3f) { crouchStart = Time.time; }}
            float dist = (Time.time - crouchStart) * m_CrouchSpeed;
            float currheight = dist / (0.8f - 0.3f);
            m_Camera.transform.localPosition = m_IsSneaking ? Vector3.Lerp(initCameraPosition, finalCameraPosition, currheight) :
